...
# 데이터 전처리 및 추천 시스템 가중치 반영
def preprocess_and_recommend(user_id, products_df, user_orders_df, user_views_df, user_cart_df):
    try:
...
# 메인 함수
def main(user_id):
    logging.info(f"회원 {user_id}에 대한 추천 작업을 시작합니다.")
    products_df, user_orders_df, user_views_df, user_cart_df = fetch_data(user_id)

    if products_df is not None and user_orders_df is not None and user_views_df is not None and user_cart_df is not None:
        recommendations = preprocess_and_recommend(user_id, products_df, user_orders_df, user_views_df, user_cart_df)

        if recommendations is not None:
            logging.info("최종 추천 상품 목록:")
            print(recommendations[['PRODUCT_ID', 'PRODUCT_NAME', 'BRAND', 'PRICE', 'weighted_score']])
        else:
            logging.error("추천 상품을 생성하는 데 문제가 발생했습니다.")
    else:
        logging.error("데이터를 가져오는 데 문제가 발생했습니다.")
