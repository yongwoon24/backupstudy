...
def save_text_to_excel(texts, file_path):
    ...
    for w in desktop.windows():
        if "Excel" in w.window_text() and w.is_visible() and w.is_enabled():
            excel_window = w
            break
    ...
    for index, text in enumerate(texts):
        for line in text.split('\n'):
            if line.strip():
                # 텍스트를 셀에 입력
                keyboard.send_keys(line, with_spaces=True)
    ...
    # 파일 경로 입력 후 저장
    save_as_window = desktop.window(title_re=".*Save As.*")
    if save_as_window.exists():
        save_as_window.type_keys(file_path + "{ENTER}", with_spaces=True)
        time.sleep(2)
    else:
        raise Exception("Save As 창을 찾을 수 없습니다.")
